-- Fix database schema by recreating rule_executions table with proper types

-- First drop the existing policy to allow column type changes
...
-- Recreate rule_executions table with TEXT rule_id to match blockchain_rule_id
...
-- Create the policy for rule_executions
...
-- Now add missing columns to programmable_rules table
DO $$ 
BEGIN
